 ...
 * @since 30.11.2021
 */
public class EnumEntriesVolatileList<E extends Enum<E>> extends AbstractList<E> {
    ...
    private volatile E[] lazyEntries;

    public EnumEntriesVolatileList(Supplier<E[]> entiresSupplier) {
        this.entiresSupplier = entiresSupplier;
    }

    private E[] getEntries() {
        E[] entries = this.lazyEntries;
        if (entries == null) {
            entries = entiresSupplier.get();
            lazyEntries = entries;
        }
        return entries;
    }
